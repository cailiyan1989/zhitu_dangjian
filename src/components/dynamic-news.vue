<template>
  <div class="dynamic-news">
    <template v-if="$route.query.category=='notice'">
      <div v-show="!noticedNewsTotal" class="showNull">
        <span>暂时没有数据。。。</span>
      </div>
    </template>
    <template v-else-if="$route.query.category=='learn'">
      <div v-show="!learnedNewsTotal" class="showNull">
        <span>暂时没有数据。。。</span>
      </div>
    </template>
    <template v-else-if="$route.query.category=='people'">
      <div v-show="!peopledNewsTotal" class="showNull">
        <span>暂时没有数据。。。</span>
      </div>
    </template>
    <template v-else-if="$route.query.category=='news'">
      <div v-show="!newedNewsTotal" class="showNull">
        <span>暂时没有数据。。。</span>
      </div>
    </template>
    <v-scroll :on-refresh="onRefresh" :on-infinite="onInfinite" :dataList="scrollData" id="newsID">
      <template v-for="(item,index) in currentNews">
        <router-link tag="div" class="list" :to="{name:'NewsDetail', params: {dynamicid: item.id, cate: $route.query.category}}"  :key="index">
          <div class="list_img">
            <span>Loading</span>
